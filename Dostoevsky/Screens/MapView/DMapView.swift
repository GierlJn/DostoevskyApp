...
struct DMapview: View {
    ...
    var body: some View {
            ...
            VStack {
                Map(coordinateRegion: $viewModel.region, showsUserLocation: true, annotationItems: viewModel.locations.filter({ loc in
                    ...
                    MapAnnotation(coordinate: location.getCLLocation().coordinate) {
                        DAnnotation(location: location, isFavorite: PersistanceManager.isFavorite(location))
                            .onTapGesture {
        ...
        .alert(item: $mapViewModel.alertItem, content: { alertItem in
...
struct PickerView: View {
    @EnvironmentObject var appViewModel: AppState
